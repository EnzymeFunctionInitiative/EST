...
#print the header
print OUT "<?xml version='1.0' ?>\n<graph id='outputFile' label='$title-$similarity repnode' xmlns='http://www.cs.rpi.edu/XGMML'>\n";
...
while (<CDHIT>){
  ...
  if($line=~/^>/){
    ...
    $nodecount++;
    print OUT "<node id=\"$head\" label=\"$head\">";
	...
	print OUT "<att type=\"string\" name=\"$key\" value=\"$piece\"/>";
      ...
      print OUT "</att>";
      #print "$key\t".join(",",@{$clusterdata{$key}})."\n";
    }
    print OUT "<att type=\"integer\" name=\"Cluster Size\" value=\"$count\"/>";
    print OUT "</node>\n";
    %clusterdata=();
    $count=0;
  }else{
    ...
    if($line=~/\*$/){
      $head=$element;
      $headuprot{$head}=1;
#      print "Head of next node is $head\n";
    }
    #print "\t$element\n";
    foreach my $key (@metas){
