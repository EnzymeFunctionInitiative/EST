...
cat <<JSON > $OUTPUT_DIR/user_filter.json
[
    {
        "name": "bacteria",
        "operator": "OR",
...
./bin/create_est_nextflow_params.py family --output-dir $OUTPUT_DIR --efi-config $EFI_CONFIG_FILE --fasta-db $EFI_FASTA_DB --efi-db $EFI_DB_NAME --families $family --sequence-version uniprot --nextflow-config $NXF_EST_CONFIG_FILE --filter user-file=$PWD/assets/predefined_taxonomy_filters.yml
