...
./bin/create_generatessn_nextflow_params.py auto --threshold-min-val 87 --ssn-name testssn --threshold-metric alignment_score --job-name test-ssn --est-output-dir $OUTPUT_DIR --nextflow-config $CONFIG_FILE --efi-config $EFI_CONFIG_FILE --efi-db $EFI_DB_NAME --color-ssn
