...
./bin/create_generatessn_nextflow_params.py auto --threshold-min-val 87 --threshold-metric bitscore --ssn-name testssn --job-name test-ssn --est-output-dir $OUTPUT_DIR --nextflow-config $CONFIG_FILE --efi-config $EFI_CONFIG_FILE --efi-db $EFI_DB_NAME
