...
./bin/create_est_nextflow_params.py fasta --output-dir $OUTPUT_DIR --efi-config $EFI_CONFIG_FILE --fasta-db $EFI_FASTA_DB --efi-db $EFI_DB_NAME --fasta-file $EFI_TEST_FASTA_FILE --nextflow-config $NXF_EST_CONFIG_FILE --families $family --family-sequence-version uniref50
