...
sub addImportOptions {
    ...
    $self->addOption("sequence-mapping-file=s", 0, "file for mapping UniProt and anonymous IDs in FASTA file (internal)", OPT_FILE);
...
sub validateOptions {
    ...
    $opts->{sequence_mapping_file} = get_default_path("sequence_mapping", $outputDir) if not $opts->{sequence_mapping_file};
