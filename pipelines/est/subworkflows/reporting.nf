...
process compute_stats {
    ...
    python $projectDir/statistics/render_boxplotstats_sql_template.py --blast-output $blast_parquet --duckdb-memory-limit "${params.duckdb_memory_limit}" --duckdb-temp-dir /scratch/duckdb-${task.hash} --boxplot-stats-output boxplot_stats.parquet --evalue-output evalue.tab --sql-template $projectDir/templates/boxplotstats-template.sql --sql-output-file boxplotstats.sql
