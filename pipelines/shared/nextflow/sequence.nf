...
process filter_ids {
    ...
    """
    perl $projectDir/../shared/import/filter_ids.pl --efi-config ${params.efi_config} --efi-db ${params.efi_db} --sequence-version ${params.sequence_version} $filter_args
    """
}
...
process get_source_ids {
    ...
    if (params.import_mode == "blast") {
        ...
        perl $projectDir/../shared/import/get_sequence_ids.pl $common_args $family_args --blast-output init_blast.out --blast-query ${params.input_file}
        """
    } else if (params.import_mode == "accessions") {
        """
        perl $projectDir/../shared/import/get_sequence_ids.pl $common_args $family_args --accessions ${params.input_file}
        """
    } else if (params.import_mode == "fasta") {
        """
        perl $projectDir/../shared/import/get_sequence_ids.pl $common_args $family_args --fasta ${params.input_file} --sequence-mapping-file seq_mapping.tab
        """
...
process get_sequences {
    ...
    if [[ -s "${accession_ids}" ]]; then
        perl $projectDir/../shared/import/get_sequences.pl --fasta-db ${fasta_db} --sequence-ids-file ${accession_ids} --output-sequence-file ${accession_ids}.fasta
    else
