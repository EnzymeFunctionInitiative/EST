...
%phylo=%efitid=%hmpdata=%gdnadata=%refseq=%GI=();
@uprotnumbers=();
...
while (<DAT>){
  ...
  if($line=~/^ID\s+(\w+)\s+(\w+);\s+(\d+)/){
	...
	if($OS=~/(\(strain.*?\))/){
      ...
      print STRUCT "$element\t$id\t$status\t$size\t$OX\t$GDNA\t$DE\t$RDE\t$OS\t$OC\t$GN\t$PFAM\t$PDB\t$IPRO\t$GO\t$giline\t$hmpsite\t$hmpoxygen\t$TID\t$EC\t$phylum\t$class\t$order\t$family\t$genus\t$species\t$cazy\n";
    }
    $id=$1;
    $status=$2;
    $size=$3;
