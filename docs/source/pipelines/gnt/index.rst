...
``bin/create_gnt_nextflow_params.py``. An example usage of the command: ::
   ...
   python bin/create_gnt_nextflow_params.py --output-dir results/ --ssn-input ssn.xgmml --efi-config efi.config --efi-db efi_db.sqlite --fasta-db uniprot.fasta

The pipeline uses the SSN to compute the GNNs. The other parameters are required
for retrieving sequences, metadata, and genome context.
