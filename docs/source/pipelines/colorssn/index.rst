...
``bin/create_colorssn_nextflow_params.py``. An example usage of the command: ::
   ...
   python bin/create_colorssn_nextflow_params.py --output-dir results/ --ssn-input full_ssn.xgmml --efi-config efi.config --efi-db efi_db.sqlite --fasta-db uniprot.fasta

The pipeline colors the input SSN and generates other outputs. The other
arguments are required for retrieving sequences and metadata.
