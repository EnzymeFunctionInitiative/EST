...
The pipeline uses the input ``id_list.txt`` file to extract genome
context from the EFIcompute the GNNs. The other parameters are required
for saving the output and specifying the data source that is used to
retrieve genome context.
