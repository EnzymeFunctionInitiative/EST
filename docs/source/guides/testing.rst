...
If the user wishes to a location for the test dataset that is outside of the
EST repository (for example, if a user storage quota limits the amount of
...
a user can control hte database interface used by the scripts: ::
...
The ``--results-dir`` option allows the user to change where output file
